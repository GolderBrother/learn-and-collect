...
function getMedia(videoDOM) {
    ...
    // console.log(navigatorMediaDevices)
    if (
 ...
 * @return
 */
function initParkOcx(parkDeviceItemList) {
    var isIE = !!window.ActiveXObject || "ActiveXObject" in window;
    var ocxZK = $("#zkVideoOcx" + " object[id='zktecoVideo']")[0];
    if (ocxZK === undefined || ocxZK === null) {
        if (isIE) {
            $("#zkVideoOcx").append('<object classid="clsid:D8AC245E-9EDA-4B5D-A8B3-3F0A2009E63B" class="videoOcx" id="zktecoVideo"  width="100%" height ="100%"></object>');
            try {
                ...
                if (parkDeviceItemList) {
                    $.each(parkDeviceItemList, function (index, devInfo) {
                        ...
                        // 1.登入句柄   2.通道名称横坐标位置  3.通道名称纵坐标位置   4.显示的通道名称
                ...
                console.log(err);
                $("#zkVideoOcx").html('<div class="zkVideoOcxTip">' +
            ...
            $("#zkVideoOcx").html('<div class="zkVideoOcxTip">' + I18n.getValue('park_preview_warn5') + '</div>');
        }
    }
}

/*获取设备并播放视频（门禁当停车）*/
function initParkDeviceAndRealPlay() {
    $.ajax({
        url: "/parkDevice.do?getOnlineByPavilioIpAddress",
